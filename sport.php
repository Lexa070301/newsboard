...
$news = mysqli_fetch_all(mysqli_query($database, 'SELECT news.id AS id, title, date FROM news INNER JOIN join_table ON (news.id = news_id) INNER JOIN categories ON (category_id = categories.id) WHERE status = "accepted" AND name = "Спорт" ORDER BY news.id DESC'), MYSQLI_BOTH);
$description = mysqli_fetch_all(mysqli_query($database, 'SELECT description FROM categories WHERE name = "Спорт"'), MYSQLI_BOTH);
$keywords = mysqli_fetch_all(mysqli_query($database, 'SELECT keyword FROM categories INNER JOIN keywords ON (categories.id = keywords.category_id) WHERE name = "Спорт"'), MYSQLI_BOTH);
